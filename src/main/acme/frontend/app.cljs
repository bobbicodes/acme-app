...
(defn range [node]
  (selection-range (start node) (end node)))
...
(defn node-at-cursor
  ...
  ([^js state from]
   (some->> (nearest-touching state from -1)
            (#(when (or (terminal-type? (type %))
                        (<= (start %) from)
                        (<= (end %) from))
                (cond-> %
                  (or (top? %)
                      (and (not (terminal-type? (type %)))
                           (< (start %) from (end %))))
                  (-> (children from -1) first))))
